...
class VARMAformer(nn.Module):
    ...
    def __init__(self, cfg: VARMAConfig):
        ...
        self.proj_out = nn.Linear(cfg.d_model, self.in_feat)
...
# ---------------------------
# Minimal training scaffold (example)
# ---------------------------

class DummyDataset(torch.utils.data.Dataset):
    """
    Example dataset: produce (x,y) pairs from a multivariate synthetic series.
    Replace with your real loader (e.g., ETT/Electricity/Traffic/Weather).
    """
    def __init__(self, length=10000, c_in=7, lookback=96, horizon=96, seed=0):
        super().__init__()
        g = torch.Generator().manual_seed(seed)
        self.C = c_in
        self.L = lookback
        self.T = horizon
        self.data = torch.randn(length + lookback + horizon, c_in, generator=g) * 0.5
        # add some periodicities
        t = torch.arange(self.data.shape[0], dtype=torch.float)
        for c in range(c_in):
            self.data[:, c] += 0.5 * torch.sin(2 * math.pi * t / (24 + c))
    ...
    def __getitem__(self, idx):
        ...
        e = idx + self.L
        f = e + self.T
        x = self.data[s:e].T.contiguous()  # (C,L)
        y = self.data[e:f].T.contiguous()  # (C,T)
        return x, y
...
class Dataset_VARMA(Dataset):
    """
    读本地 CSV, 按 [seq_len, label_len(占位), pred_len] 切片，
    直接返回:
      x: (C_in, L)
      y: (C_out, T)
    - features='M': 多变量输入→多变量输出忽略 target
    - features='S': 单变量输入→单变量输出(只用 target)
    - features='MS': 多变量输入→单变量输出(输出只用 target)
    """
    def __init__(self, root_path, flag='train', size=None,
                 features='M', data_path='ETTh1.csv',
                 target='OT', scale=False, cols=None):
        if size is None:
            self.seq_len  = 24*4*4
            self.label_len = 24*4   # 占位，不用
            self.pred_len = 24*4
        else:
    ...
    def _read_data(self):
        ...
        # 输入列候选：用户给了 cols 就用；否则用除 'date' 外的所有列
        if self.cols:
            in_cols = self.cols.copy()
        else:
            in_cols = [c for c in df_raw.columns if c != 'date']

        # features 分支
        if self.features == 'M':
            # 多变量→多变量：不需要 target，直接用 in_cols
            keep_cols = (['date'] if has_date else []) + in_cols
            df_raw = df_raw[keep_cols]
            df_in  = df_raw.iloc[:, 1:] if has_date else df_raw  # 去掉 date
            df_out = df_in

        elif self.features == 'S':
            # 单变量→单变量：必须有 target
            if self.target not in df_raw.columns:
                raise KeyError(f"target='{self.target}' 不在 CSV 列中，现有列：{list(df_raw.columns)}")
            keep_cols = (['date'] if has_date else []) + [self.target]
            df_raw = df_raw[keep_cols]
            df_in  = df_raw[[self.target]]
            df_out = df_in

        elif self.features == 'MS':
            # 多变量→单变量：输入=所有数值列，输出=target
            if self.target not in df_raw.columns:
                raise KeyError(f"target='{self.target}' 不在 CSV 列中，现有列：{list(df_raw.columns)}")
            keep_cols = (['date'] if has_date else []) + in_cols
            # 确保 target 在输入里也存在(若不在 in_cols，则附加)
            if self.target not in keep_cols:
                keep_cols = (['date'] if has_date else []) + in_cols + [self.target]
            df_raw = df_raw[keep_cols]
            df_in  = df_raw.iloc[:, 1:] if has_date else df_raw
            df_out = df_raw[[self.target]]
        else:
        ...
        if self.scale:
            ...
            tr_x = df_in.iloc[border1s[0]:border2s[0]].values
            tr_y = df_out.iloc[border1s[0]:border2s[0]].values
            self.scaler_x.fit(tr_x); self.scaler_y.fit(tr_y)
            data_x = self.scaler_x.transform(df_in.values)
            data_y = self.scaler_y.transform(df_out.values)
        else:
            self.scaler_x = None
            self.scaler_y = None
            data_x = df_in.values
            data_y = df_out.values

        # 当前子集的数据
        self.data_x = data_x[b1:b2]  # (Len_sub, C_in)
        self.data_y = data_y[b1:b2]  # (Len_sub, C_out)

        self.C_in  = self.data_x.shape[1]
        self.C_out = self.data_y.shape[1]
    ...
    def __getitem__(self, idx):
        ...
        e = s + self.seq_len
        f = e + self.pred_len
        x = torch.as_tensor(self.data_x[s:e], dtype=torch.float32).transpose(0, 1).contiguous()  # (C_in, L)
        y = torch.as_tensor(self.data_y[e:f], dtype=torch.float32).transpose(0, 1).contiguous()  # (C_out, T)
        return x, y

    def inverse_transform_y(self, data_np):
        if self.scaler_y is None:
            return data_np
        return self.scaler_y.inverse_transform(data_np)
...
if __name__ == "__main__":
    ...
    train_model(
            ...
            grad_clip=1.0,
            patience=5,
            save_path='./miniQMT/DL/VARMA/varmaformer_best_dummy_data.pth'
        )
