...
class OptionTrainingDataGenerator:
    def __init__(self, window_size=32, predict_horizon=1, ks_range=(0.9, 1.1), min_ratio: float=0.8):
        ...
        self.mask_feature_dim = 6 # 隐含波动率 + 5个希腊字母
        self.total_feature_dim = 9 # 总特征维度
        self.mask_cols = [
            '隐含波动率', 
    ...
    def load_and_process_file(self, file_path: str, tolerate: int = None):
        ...
        mask_cols_to_zero = ['隐含波动率', 'Delta', 'Gamma', 'Vega', 'Theta', 'Rho']
    ...
    # 直接获取最终的loader
    def get_data_loader(self, train_size: float=0.7, val_size: float=0.2, file_list: str=None, batch_size: int=64):
        ...
        valid_loader = DataLoader(TensorDataset(X_val, Y_val), batch_size=batch_size, shuffle=False)
