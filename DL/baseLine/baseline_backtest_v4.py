...
def is_listed_on(account: single_Account, code: str, ts_str: str) -> bool:
    """
    判断合约在 ts_str 时点是否已上市（openDate <= ts_str）。
    option_info_controller.get_openDate(code) 返回字符串（通常形如 YYYYMMDD 或带分隔符）。
    - 若 openDate / ts_str 都能解析出 >=14 位数字，则比较到秒（YYYYMMDDHHMMSS）
    - 否则比较日期（YYYYMMDD）
    解析失败则保守返回 False。
    """
...
def run_backtest(
    ...
    account.init_hv160(start_time, end_time, '510050')
    ...
    for row in df.itertuples(index=False):
        ...
        # ===== debug: only first 200 bars =====
        if cnt < 200:
            iv = 0.5*(account.comb.get("call_iv",0.0)+account.comb.get("put_iv",0.0))
            hv = float(account.get_hv_160(ts_str) or 0.0)
            if iv>0 and hv>0:
                ivs.append(iv); hvs.append(hv)
        elif cnt == 200 and ivs:
            print("IV sample:", np.percentile(ivs,[5,50,95]))
            print("HV sample:", np.percentile(hvs,[5,50,95]))
