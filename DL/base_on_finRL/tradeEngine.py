...
class single_Account:
    ...
    def getReward(self, action: int, eps: float=1e-6):
        ...
        if cur > self.init_capital and step_ret < 0:
            final_reward += step_ret * 60.0 # 额外增加 60 倍权重的回吐惩罚
        ...
        if cur_dd > prev_dd:
            # 只要回撤在扩大，就根据回撤增量施加重罚
            final_reward -= (cur_dd - prev_dd) * 500.0 # 权重从 50 提至 100，压制 30w-50w 的巨震
        ...
        # 7. Reward Clipping
        final_reward = np.clip(final_reward, -2, 2)
