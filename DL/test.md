...
# 基于 PAB-VarTST 与 Skill-PPO 的期权交易策略研究
## 组会汇报与工程思路总结 | [您的姓名] | [日期]
...
| **Transformer 退化 (Why Attention Fails)** | **结构先验融合**：结合 **VARMA (局部先验)** 和 **BasisFormer (全局结构)**，避免 Attention 机制失效。 | 鲁棒、可解释、高泛化性的特征。 |
...
* **架构思路 (CDT-PPO-MOE)**：
    ...
    2.  **分层执行 (MoE)**：
        * **Manager**：根据市场特征动态选择**交易技能/专家 (Expert)** $z$。
...
* **VARMAformer 预训练**：独立训练和加载测试通过（如日志所示），可作为 PAB-VarTST 的基础组件。
...
3.  **消融实验准备**：构建消融实验所需的训练脚本和数据切片，包括“无预训练” vs “完整 PAB-VarTST” 等。